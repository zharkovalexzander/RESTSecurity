...
import springapp.config.SpringConfigure;
...
public class CountryDAO extends SpringConfigure implements CountryBehavior {
    ...
    @Override
    @Transactional
    public void persist(CountryEntity record) {
        Session session = this.sessionFactory.getCurrentSession();
        Transaction transaction = session.beginTransaction();
        try {
            transaction.begin();
    ...
    @Override
    public Integer visitCountry(String code) {
        Session session = this.sessionFactory.getCurrentSession();
        Query query = session.createQuery("UPDATE CountryEntity SET visited = :visited WHERE code LIKE :code");
        query.setParameter("code", code);
        query.setParameter("visited", 1);
